...
export default function Footer() {
    ...
    return (
        <footer className="bg-[#263238] text-gray-300">
            <div className="max-w-7xl mx-auto py-14 grid md:grid-cols-2 gap-10 px-6">

                {/* LOGO & ABOUT */}
                <div>
                    <p className="text-xs mb-1">Powered by</p>
                    <div className="flex items-center gap-2 mb-4">
                        <img src={logo} alt="Oyster Technologies" className="w-14 h-14 bg-white rounded-full" />
                        <div>
                    ...
                    <div className="flex gap-3">
                        {footerLinks.social.map((item) => (
                            <a key={item.name} href="#" className="p-2 bg-gray-700 rounded hover:bg-blue-600 transition-colors">
                                {item.icon}
                            </a>
                        ))}
                    </div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-2 text-sm">
                            {footerLinks.platform.map((item) => (
                                <li key={item.name}>
                                    <NavLink to={item.href} className="hover:text-blue-400 transition-colors">
                                        {item.name}
                                    </NavLink>
                                </li>
                            ))}
            ...
            <div className="border-t border-gray-700 py-4 text-center text-sm text-gray-400">
                © {currentYear} Oyster Technology. All rights reserved.
            </div>
        </footer>
    );
}
